...
Forward search and inverse search are available: in the GIF I use the keybinding ~C-c C-v~ to jump from the LaTeX buffer to the PDF and sync the location (note the little red arrow and the highlight in the text); double-clicks on the PDF will activate the backward search.
